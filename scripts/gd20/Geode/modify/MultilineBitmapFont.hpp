#pragma once
#include <Geode/modify/Modify.hpp>
#include <Geode/modify/Field.hpp>
#include <Geode/binding/MultilineBitmapFont.hpp>
using namespace geode::modifier;
namespace geode::modifier {
    
	#ifndef GEODE_STATICS_createWithFont
		#define GEODE_STATICS_createWithFont
		GEODE_AS_STATIC_FUNCTION(createWithFont) 
	#endif

	#ifndef GEODE_STATICS_getIgnoreColorCode
		#define GEODE_STATICS_getIgnoreColorCode
		GEODE_AS_STATIC_FUNCTION(getIgnoreColorCode) 
	#endif

	#ifndef GEODE_STATICS_getSizeWidth
		#define GEODE_STATICS_getSizeWidth
		GEODE_AS_STATIC_FUNCTION(getSizeWidth) 
	#endif

	#ifndef GEODE_STATICS_getTextHeight
		#define GEODE_STATICS_getTextHeight
		GEODE_AS_STATIC_FUNCTION(getTextHeight) 
	#endif

	#ifndef GEODE_STATICS_getTextPos
		#define GEODE_STATICS_getTextPos
		GEODE_AS_STATIC_FUNCTION(getTextPos) 
	#endif

	#ifndef GEODE_STATICS_getTextWidth
		#define GEODE_STATICS_getTextWidth
		GEODE_AS_STATIC_FUNCTION(getTextWidth) 
	#endif

	#ifndef GEODE_STATICS_initWithFont
		#define GEODE_STATICS_initWithFont
		GEODE_AS_STATIC_FUNCTION(initWithFont) 
	#endif

	#ifndef GEODE_STATICS_readColorInfo
		#define GEODE_STATICS_readColorInfo
		GEODE_AS_STATIC_FUNCTION(readColorInfo) 
	#endif

	#ifndef GEODE_STATICS_setIgnoreColorCode
		#define GEODE_STATICS_setIgnoreColorCode
		GEODE_AS_STATIC_FUNCTION(setIgnoreColorCode) 
	#endif

	#ifndef GEODE_STATICS_stringWithMaxWidth
		#define GEODE_STATICS_stringWithMaxWidth
		GEODE_AS_STATIC_FUNCTION(stringWithMaxWidth) 
	#endif

	#ifndef GEODE_STATICS_setOpacity
		#define GEODE_STATICS_setOpacity
		GEODE_AS_STATIC_FUNCTION(setOpacity) 
	#endif

    
	#ifndef GEODE_CONCEPT_CHECK_createWithFont
		#define GEODE_CONCEPT_CHECK_createWithFont
		GEODE_CONCEPT_FUNCTION_CHECK(createWithFont) 
	#endif

	#ifndef GEODE_CONCEPT_CHECK_getIgnoreColorCode
		#define GEODE_CONCEPT_CHECK_getIgnoreColorCode
		GEODE_CONCEPT_FUNCTION_CHECK(getIgnoreColorCode) 
	#endif

	#ifndef GEODE_CONCEPT_CHECK_getSizeWidth
		#define GEODE_CONCEPT_CHECK_getSizeWidth
		GEODE_CONCEPT_FUNCTION_CHECK(getSizeWidth) 
	#endif

	#ifndef GEODE_CONCEPT_CHECK_getTextHeight
		#define GEODE_CONCEPT_CHECK_getTextHeight
		GEODE_CONCEPT_FUNCTION_CHECK(getTextHeight) 
	#endif

	#ifndef GEODE_CONCEPT_CHECK_getTextPos
		#define GEODE_CONCEPT_CHECK_getTextPos
		GEODE_CONCEPT_FUNCTION_CHECK(getTextPos) 
	#endif

	#ifndef GEODE_CONCEPT_CHECK_getTextWidth
		#define GEODE_CONCEPT_CHECK_getTextWidth
		GEODE_CONCEPT_FUNCTION_CHECK(getTextWidth) 
	#endif

	#ifndef GEODE_CONCEPT_CHECK_initWithFont
		#define GEODE_CONCEPT_CHECK_initWithFont
		GEODE_CONCEPT_FUNCTION_CHECK(initWithFont) 
	#endif

	#ifndef GEODE_CONCEPT_CHECK_readColorInfo
		#define GEODE_CONCEPT_CHECK_readColorInfo
		GEODE_CONCEPT_FUNCTION_CHECK(readColorInfo) 
	#endif

	#ifndef GEODE_CONCEPT_CHECK_setIgnoreColorCode
		#define GEODE_CONCEPT_CHECK_setIgnoreColorCode
		GEODE_CONCEPT_FUNCTION_CHECK(setIgnoreColorCode) 
	#endif

	#ifndef GEODE_CONCEPT_CHECK_stringWithMaxWidth
		#define GEODE_CONCEPT_CHECK_stringWithMaxWidth
		GEODE_CONCEPT_FUNCTION_CHECK(stringWithMaxWidth) 
	#endif

	#ifndef GEODE_CONCEPT_CHECK_setOpacity
		#define GEODE_CONCEPT_CHECK_setOpacity
		GEODE_CONCEPT_FUNCTION_CHECK(setOpacity) 
	#endif


	template<class Der>
	struct ModifyDerive<Der, MultilineBitmapFont> : ModifyBase<ModifyDerive<Der, MultilineBitmapFont>> {
        using BaseModify = ModifyBase<ModifyDerive<Der, MultilineBitmapFont>>;
		using ModifyBase<ModifyDerive<Der, MultilineBitmapFont>>::ModifyBase;
		using Base = MultilineBitmapFont;
        using Derived = Der;
		void apply() override {

            GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR_DEFINED(MultilineBitmapFont, createWithFont, char const*, gd::string, float, float, cocos2d::CCPoint, int, bool)
            GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR_DEFINED(MultilineBitmapFont, getIgnoreColorCode, )
            GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR_DEFINED(MultilineBitmapFont, getSizeWidth, )
            GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR_DEFINED(MultilineBitmapFont, getTextHeight, )
            GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR_DEFINED(MultilineBitmapFont, getTextPos, )
            GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR_DEFINED(MultilineBitmapFont, getTextWidth, )
            GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR_DEFINED(MultilineBitmapFont, initWithFont, char const*, gd::string, float, float, cocos2d::CCPoint, int, bool)
            GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR_DEFINED(MultilineBitmapFont, readColorInfo, gd::string)
            GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR_DEFINED(MultilineBitmapFont, setIgnoreColorCode, bool)
            GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR_DEFINED(MultilineBitmapFont, stringWithMaxWidth, gd::string, float, float)
            GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR_DEFINED(MultilineBitmapFont, setOpacity, unsigned char)
		}
	};
}
