...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, LoadingLayer> : ModifyBase<ModifyDerive<Der, LoadingLayer>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x119690, Membercall, LoadingLayer, getLoadingString, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x1187f0, Membercall, LoadingLayer, init, bool)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x1190c0, Membercall, LoadingLayer, loadAssets, )
            GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR_DEFINED(LoadingLayer, loadingFinished, )
            GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR_DEFINED(LoadingLayer, scene, bool)
            GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR_DEFINED(LoadingLayer, updateProgress, int)
		}
	};
}
